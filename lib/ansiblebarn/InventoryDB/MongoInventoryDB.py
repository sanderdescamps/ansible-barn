...
class MongoInventoryDB(InventoryDB):
    def __init__(self, hostname, port=27017, username=None, password=None):
        ...
        if username is not None and password is not None:
            self.mdb = pymongo.MongoClient("mongodb://%s:%s@%s:%s/" % (urllib.parse.quote_plus(
                self.username), urllib.parse.quote_plus(self.password), self.hostname, str(self.port)))
        else:
            self.mdb = pymongo.MongoClient(
                "mongodb://%s:%s/" % (self.hostname, str(self.port)))
    ...
    def add_child_group_to_group(self, child_group, parent_group):
        self.mdb["inventory"]["group_inventory"].update_one(
            {"name": child_group}, {"$addToSet": {"parent_groups": parent_group}})
        self.mdb["inventory"]["group_inventory"].update_one(
            {"name": parent_group}, {"$addToSet": {"child_groups": child_group}})

    def add_host_to_group(self, host_name, group_name):
        if self.host_exist(host_name):
            self.mdb["inventory"]["group_inventory"].update_one(
                {"name": group_name}, {"$addToSet": {"hosts": host_name}})
            self.mdb["inventory"]["host_inventory"].update_one(
                {"name": host_name}, {"$addToSet": {"groups": group_name}})
        else:
            raise MemberNotFoundError

    def set_variable(self, name, key, value):
        if self.host_exist(name):
            v = self.mdb["inventory"]["host_inventory"].find_one(
                {"name": name}).get("vars", {})
            v[key] = value
            self.mdb["inventory"]["host_inventory"].update_one(
                {"name": name}, {"$set": {"vars": v}})
        elif self.group_exist(name):
            v = self.mdb["inventory"]["group_inventory"].find_one(
                {"name": name}).get("vars", {})
            v[key] = value
            self.mdb["inventory"]["group_inventory"].update_one(
                {"name": name}, {"$set": {"vars": v}})
    ...
    def delete_variable(self, name, key):
        if self.host_exist(name):
            v = self.mdb["inventory"]["host_inventory"].find_one({"name": name})[
                "vars"]
            del v[key]
            self.mdb["inventory"]["host_inventory"].update_one(
                {"name": name}, {"$set": {"vars": v}})
        elif self.group_exist(name):
            v = self.mdb["inventory"]["group_inventory"].find_one({"name": name})[
                "vars"]
            del v[key]
            self.mdb["inventory"]["group_inventory"].update_one(
                {"name": name}, {"$set": {"vars": v}})
    ...
    def get_vars(self, name):
        res = {}
        if self.host_exist(name):
            host = self.mdb["inventory"]["host_inventory"].find_one({
                                                                    "name": name})
            ...
            while len(to_process_groups) > 0:
                ...
                g = self.mdb["inventory"]["group_inventory"].find_one(
                    {"name": g_name})
    ...
    def export(self, name=None):
        if name is None or name.lower() == "all":
            ...
            hosts = list(self.mdb["inventory"]
                         ["host_inventory"].find({"name": name}))
            hosts = remove_underscore_keys(hosts)
            return {"hosts": hosts}
        elif self.group_exist(name):
            groups = list(self.mdb["inventory"]
                          ["group_inventory"].find({"name": name}))
            groups = remove_underscore_keys(groups)
